...
string LoadRandomWord(string path) {
    ...
    if (file.is_open()) {
        ...
        while (getline(file, word)) {
            if (!word.empty()) {
                // Convert word to uppercase and remove any whitespace
    ...
    if (words.empty()) {
        ...
        cout << "Warning: Using default word list (couldn't open words.txt)" << endl;
    }
...
int main() {
    ...
    do {
        ...
        if (win) break;
        if (tries >= 9) break;  // Stop if reached maximum tries
